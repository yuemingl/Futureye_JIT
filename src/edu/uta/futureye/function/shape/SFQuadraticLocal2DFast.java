 ...
 *
 */
public class SFQuadraticLocal2DFast implements ShapeFunction {
	...
	protected class SF123456 extends FAbstract {
		int funIndex;
		...
		public FunctionDerivable derivative(DerivativeIndicator di) {
			...
			if(di.get().keySet().contains("x")) {
				if(funIndex == 0)
					...
					return FOBasicDerivable.LinearCombination(4.0*dlx[0], fs, 4.0*dlx[1], fr);
				else if(funIndex == 4)
					return FOBasicDerivable.LinearCombination(4.0*dlx[1], ft, 4.0*dlx[2], fs);
				else if(funIndex == 5)
					return FOBasicDerivable.LinearCombination(4.0*dlx[0], ft, 4.0*dlx[2], fr);		
				else {
					FutureEyeException e = new FutureEyeException("Error: SF123456.derivative(x)");
					e.printStackTrace();
				}			
			} else if(di.get().keySet().contains("y")) {
				if(funIndex == 0)
					...
					return FOBasicDerivable.LinearCombination(4.0*dly[0], fs, 4.0*dly[1], fr);
				else if(funIndex == 4)
					return FOBasicDerivable.LinearCombination(4.0*dly[1], ft, 4.0*dly[2], fs);
				else if(funIndex == 5)
					return FOBasicDerivable.LinearCombination(4.0*dly[0], ft, 4.0*dly[2], fr);		
				else {
					FutureEyeException e = new FutureEyeException("Error: SF123456.derivative(y)");
					e.printStackTrace();
				}
			} else {
				FutureEyeException e = new FutureEyeException("Error: SF123456.derivative()");
				e.printStackTrace();				
			}
			return null;
		}
		...
		public double value(Variable v) {
			...
			if(funIndex == 0)
				...
				FutureEyeException e = new FutureEyeException("Error: SF123456.value()");
				e.printStackTrace();
			}
			return 0.0;
		}
	...
	//TODO ??? 应该采用一维二次型函数
	ShapeFunction sf1d1 = new SFQuadraticLocal1D(1);
	ShapeFunction sf1d2 = new SFQuadraticLocal1D(2);
	ShapeFunction sf1d3 = new SFQuadraticLocal1D(3);
	@Override
	public ShapeFunction restrictTo(int funIndex) {
		if(funIndex == 1) return sf1d1;
			...
			FutureEyeException e = new FutureEyeException("Error: SF123456.restrictTo()");
			e.printStackTrace();
			return null;
		}
	}
