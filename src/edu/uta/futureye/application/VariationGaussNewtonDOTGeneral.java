...
//	}
	...
	public static MathFun generateTestRealMu_a(double max, double bk) {
	...
	public static MathFun generateTestGuessMu_a(double max, double bk) {
		final double fmax = max;
		final double fbk = bk;
		return new AbstractMathFun("x","y"){
			@Override
...
//		weakForm.setF(f2);
		...
		MathFun akmk_2mk = FMath.pow(new Vector2Function(ak).M(model_k),-2.0)
							.M(model_k).M(1.0/beta);
		//Function ak_2 = FMath.pow(new Vector2Function(ak),-2.0).M(1.0/beta);
		int NF = u.length;
		MathFun[] fu = new MathFun[NF];
		MathFun[] fl = new MathFun[NF];
		MathFun[] fumfl = new MathFun[NF];
		DuDn[] du0dn = new DuDn[NF];
		MathFun[] du0dnmfl = new MathFun[NF];
		for(int k=0;k<NF;k++) {
...
//        		b2.M(_amk_2mk));
        ...
        MathFun _amk_2mk = FMath.pow(fa.M(model_k),-2).M(model_k).M(-1.0);
...
//        		b2.M(_amk_2mk));
	...
	public Equation testGetCTLoad(Vector[] u, Vector[] u_x,Vector[] u_y, Vector[] lambda, 
			Vector ak, MathFun diri) {
		
		MathFun akmk_2mk = FMath.pow(new Vector2Function(ak).M(model_k),-2.0).M(model_k).M(-1.0/beta);
		int NF = u.length;
		MathFun[] fu = new MathFun[NF];
		MathFun[] fl = new MathFun[NF];
		for(int k=0;k<NF;k++) {
...
//		plotVector(mesh, FMath.axpy(-1.0, uGuessDiriReal,uGuess), String.format("M%02d_uGuessDiriReal_uGuess_diff.dat",s_i));
	...
	public void imposeDirichletCondition(SparseBlockMatrix BM, SparseBlockVector BV,
			int nDataBlock, MathFun[] u, MathFun[] g, MathFun[] lambda) {
		ElementList eList = mesh.getElementList();
		int nNode = mesh.getNodeList().size();
		for(int i=1;i<=eList.size();i++) {
			Element e = eList.at(i);
...
//	}
	...
	public Vector clearInnerValues(Mesh mesh, Vector v) {
		HashMap<NodeType, MathFun> mapNTF = new HashMap<NodeType, MathFun>();
		mapNTF.put(NodeType.Robin, null);
		Map<NodeType, MathFun> mapNTFOld = mesh.getMarkBorderMap(); 
	...
	public List<ParamOfLightSource> generateSimulateData() {
		...
		for(int i=0; i<LSx.length; i++) {
			...
			if(debug) {
				...
				MathFun gx = new DuDx(mesh,new Vector2Function(para.g,mesh,"x","y"),"x");
				MathFun gy = new DuDx(mesh,new Vector2Function(para.g,mesh,"x","y"),"y");
